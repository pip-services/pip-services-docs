...
Now let's add a **SimpleService** component with a **greeting()** method. This method will perform just one simple operation - output a message that was generated by one of the Worker controllers. This component will be implementing the **IReferenceable** interface, and its **set_references** method will receive and set a reference to the Worker controller via the 111 locator. We can also implement **IUnreferenceable** and use the **unset_references** method to delete the previously set reference.
